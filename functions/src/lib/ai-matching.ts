 ...
 * AI 매칭 시스템 (v1.0)
 *
 * @deprecated 2025-11-07 - 랜덤 매칭 시스템(v2.0)으로 대체됨
 *
 * 이 파일은 나중에 재사용 가능성을 위해 보존됩니다.
 * 새로운 매칭 로직은 random-matching.ts를 참고하세요.
 *
 * 변경 사유:
 * - AI 분석 제거 → 랜덤 전달
 * - 프로필북 개수: 누적 인증 기반 (2 × (누적인증 + 1))
 * - 성별 균형 우선
 * - 최근 3일 중복 방지
 */
...
`;
    ...
    // @ts-ignore - AI SDK 타입 호환성 문제 우회
    const result: any = await generateObject({
      model: model as any,
      schema: matchingResponseSchema as any,
 ...
 *
 * @deprecated 2025-11-07 - 랜덤 매칭 시스템으로 대체됨 (random-matching.ts)
 */
