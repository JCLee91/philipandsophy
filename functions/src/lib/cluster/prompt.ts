...
4. 🚨 **[필수] 단일 성별 클러스터 절대 금지 (위반 시 전체 결과 무효)**
   - 모든 클러스터에 남성과 여성이 **최소 1명씩 반드시** 포함되어야 합니다.
   - 남성만 또는 여성만으로 구성된 클러스터는 **절대 생성 금지**입니다.
   - 주제 유사성보다 성비 균형이 **더 중요**합니다. 주제가 약간 맞지 않더라도 성비를 맞추세요.
   - 이 규칙을 위반하면 전체 클러스터링 결과가 무효 처리되어 재생성됩니다.
...
- reasoning은 해요체로 3~4문장으로 작성 (친근하고 자연스럽게, 구체적으로)
...
- reasoning은 해요체로 3~4문장 작성 ("~입니다" ❌, "~해요" ✅)
  - 1문장: 공통점의 핵심을 설명
  - 2~3문장: 구체적인 근거와 맥락 제시
  - 친근하고 자연스러운 표현 선호
- 예시:
  - theme: "삶의 본질에 대한 공통된 고민" ✅
  - reasoning: "삶의 본질에 대해 함께 고민해요. 일상 속에서 진정한 의미를 찾고자 노력하고 있어요. 각자의 경험을 통해 존재의 가치를 성찰하고 있어요." ✅
`;
...
✓ 중복된 participantId가 없는가?
✓ 모든 participantId가 포함되었는가?
✓ 🚨 각 클러스터에 남성과 여성이 최소 1명씩 포함되어 있는가? (단일 성별 클러스터 = 무효)
`.trim();
    }
