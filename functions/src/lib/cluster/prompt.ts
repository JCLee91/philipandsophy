...
**오늘 데이터에서 가장 두드러지는 공통점**을 찾아 축으로 삼으세요.

### 축 예시 (참고용, 이 외에도 자유롭게!)
- 인간관계, 성장/자기개발, 일상/여유, 자유/독립
- 요즘의 감정, 관심 주제, 생각의 관점
- 글쓰기 스타일, 독서 감상
- **또는** 오늘 데이터에서 발견한 새로운 축!

### ⚠️ 축 선택 규칙
1. **구체적인 축**으로 정하세요
   - ❌ "가치관" (너무 포괄적, 사용 금지)
   - ✅ "인간관계", "성장", "일상", "위로" 등 구체적인 키워드
2. 오늘 데이터에서 **실제로 패턴이 보이는** 축만 선택
3. 그 축으로 묶었을 때 **서로 대화가 통할 것 같은** 그룹이 만들어지는지
4. 데이터를 보고 자유롭게 축을 만들어도 됩니다 (예시에 얽매이지 마세요)
...
**어제 사용된 축: [${recentCategories.slice(0, 3).join(', ')}]**

⛔ **위 축들은 오늘 사용하지 마세요!**
- 유저들이 매일 다른 관점으로 매칭되길 원합니다
- 어제와 같은 축을 쓰면 신선함이 떨어집니다
- 반드시 **다른 축**을 선택해주세요

✅ 다른 축을 찾아보세요:
- 어제 "인간관계"였다면 → "성장", "요즘의 감정", "관심 주제" 등
- 어제 "감정" 관련이었다면 → "일상/여유", "글쓰기 스타일", "독서 감상" 등
- 또는 오늘 데이터에서 새로운 축을 발견해보세요!

💡 정말 다른 축에서 패턴을 찾기 어렵다면, 같은 대분류여도 **다른 세부 관점**을 찾으세요.
`.trim();
}
...
- **category**: 선택한 축의 **구체적인** 키워드 (2~6글자)
  - 좋은 예: "인간관계", "성장", "일상", "자유", "위로", "관심 주제"
  - ❌ 피할 것: "가치관" (너무 포괄적, 절대 사용 금지)
