...
| Stage | 목표 | 핵심 산출물 | 현황 |
|-------|------|-------------|------|
| Stage 1 | 레이아웃·인증 경계 재정비 | 서버 레이아웃 전환, Devtools lazy 로딩, 라우팅 보호 강화 | ✅ 완료 |
| Stage 2 | 공용 레이어 구축 & 경량 보안 점검 | 컴포넌트/훅 인벤토리, 도메인별 폴더링, Firestore Emulator 스모크 테스트 | ✅ 완료 |
| Stage 3 | 도메인 구조 분리 & 운영 안정화 | `/app/chat` 분리 + Route Handler, 기타 화면 정리, Worker/스크립트 개선 | ✅ 완료 |
