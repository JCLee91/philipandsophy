# Walkthrough - Push Notification Title for Notices (Archive)

이 문서는 특정 변경 작업의 구현 기록입니다. 현재 기능/구현은 코드와 `docs/` 문서를 기준으로 확인하세요.
...
#### `src/app/datacntr/notices/create/page.tsx`
...
#### `src/app/api/datacntr/notices/create/route.ts`
-   Updated to parse the `title` from `FormData` and save it to Firestore.

#### `src/app/api/datacntr/notices/[noticeId]/route.ts`
