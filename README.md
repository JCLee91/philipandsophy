...
### 📦 책 메타데이터 자동 저장 (11개 주요 버그 수정)

#### 데이터 무결성 & 동시성
1. **Firebase Transaction 레이스 컨디션 방지** - `runTransaction()`으로 원자적 업데이트 보장
2. **수동 입력 시 메타데이터 손실 경고** - `window.confirm()`으로 사용자 확인 요청
3. **null/undefined 일관성** - 전체 코드베이스에서 `undefined` 사용 통일
4. **동시 업데이트 충돌 방지** - Firebase 트랜잭션으로 동시성 보장

#### API & 네트워크
5. **API 요청 취소 메커니즘** - abort flag 패턴으로 불필요한 요청 차단
