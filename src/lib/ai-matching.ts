...
/**
 * 여러 참가자 그룹에서 AI 매칭을 수행합니다.
 * 한 참가자가 여러 번 선택되지 않도록 관리합니다.
 */
export async function matchMultipleGroups(
  question: string,
  allParticipants: ParticipantAnswer[],
  groupSize: number = 4
): Promise<MatchingResult[]> {
  ...
  while (availableParticipants.length >= groupSize) {
    // 아직 선택되지 않은 참가자만 필터링
    const candidates = availableParticipants.filter((p) => !selectedIds.has(p.id));

    if (candidates.length < groupSize) {
      logger.warn('매칭할 수 있는 참가자가 부족합니다.');
      break;
    }
    ...
    availableParticipants.splice(
      ...
      ...availableParticipants.filter((p) => !matchedIds.has(p.id))
    );
  }
