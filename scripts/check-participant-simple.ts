...
async function checkParticipant() {
  try {
    ...
    // 2. dailyFeaturedParticipants 직접 확인
    console.log('\n2️⃣ dailyFeaturedParticipants 할당:');
    ...
    console.log(`   타입: ${assigned.length % 2 === 0 ? '✅ 짝수' : '⚠️  홀수'}`);
    ...
    if (uniqueIds.size !== assigned.length) {
      console.log(`   ⚠️  중복 발견!`);
      console.log(`   유니크: ${uniqueIds.size}개`);
      console.log(`   전체: ${assigned.length}개`);
      ...
      console.log(`   ✅ 중복 없음 (${uniqueIds.size}개 유니크)`);
    }
